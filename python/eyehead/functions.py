...
def plot_stim_locked_saccade_probability(
    go_frame,
    go_direction_x,
    go_direction_y,
    saccade_frames,
    ...
    """
    Plot stimulus-locked saccade probability grouped by direction and save the figure.

    Parameters
    ----------
    go_frame : array-like
        Stimulus onset frames.
    go_direction_x, go_direction_y : array-like
        Direction components for each stimulus.
    saccade_frames : array-like
        Frame indices of detected saccades.
    session_name : str
        Identifier used in the output filename.
    eye_name : str
        Eye label used in the output filename.
    results_dir : Path
        Directory where the figure will be saved.
    ttl_freq : int, optional
        Frame rate of the TTL signal in Hz. Defaults to 60.
    t_window_s : float, optional
        Time window on either side of the stimulus in seconds. Defaults to 2.
    bin_ms : float, optional
        Width of histogram bins in milliseconds. Defaults to 50.
    total_duration_frames : int, optional
        Total recording duration in frames. If None, inferred from `saccade_frames`.
    ...
    """
    go_frame = np.asarray(go_frame).flatten()
    saccade_frames = np.asarray(saccade_frames).flatten()
    ...
    baseline_rate = len(saccade_frames) / total_bins
    ...
    def peristim_change(stim_frames):
        ...
        prob_bin = counts / len(stim_frames)
        return 100 * (prob_bin - baseline_rate) / baseline_rate
    ...
    fig.text(0.01, 0.5, "% Change ", va="center", rotation="vertical", fontsize=11)
    fig.tight_layout()
