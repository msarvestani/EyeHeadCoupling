...
def extract_trial_trajectories(eot_df: pd.DataFrame, eye_df: pd.DataFrame,
    ...
    Trial timing is determined from vstim_cue (target_df):
    - Trial start: Each successive row in vstim_cue
    - ITI: Minimum difference between consecutive vstim_cue timestamps, rounded down
    - Trial end: trial_end(i) = trial_start(i) + (trial_start(i+1) - ITI)

    Parameters
    ----------
    eot_df : pd.DataFrame
        End of trial data (used as fallback for last trial)
    eye_df : pd.DataFrame
        Eye position data (cleaned, no duplicates)
    target_df : pd.DataFrame
        Target position data (vstim_cue)
    ...
    if n_trials > 1:
        ...
        ITI = int(np.floor(min_diff))  # Round down to nearest whole number
        print(f"\nInferred ITI: {ITI} seconds (from minimum inter-cue interval: {min_diff:.3f}s)")
    else:
        ITI = 0
        print(f"\nWarning: Only 1 trial, cannot infer ITI")

    for i in range(n_trials):
        trial_num = i + 1

        # Trial starts when target appears (vstim_cue)
        ...
        # Calculate trial end: trial_end(i) = trial_start(i) + (trial_start(i+1) - ITI)
        if i < n_trials - 1:
            # Not the last trial
            next_trial_start = target_df.iloc[i+1]['timestamp']
            end_time = start_time + (next_trial_start - start_time - ITI)

            # Find corresponding end_frame in eye_df
            # Find the frame closest to end_time
            time_diffs_to_end = np.abs(eye_df['timestamp'].values - end_time)
            closest_idx = np.argmin(time_diffs_to_end)
            end_frame = eye_df.iloc[closest_idx]['frame']
        else:
            # Last trial: use eot_df if available, otherwise use a default duration
            if len(eot_df) >= n_trials and i < len(eot_df):
                end_frame = eot_df.iloc[i]['frame']
                end_time = eot_df.iloc[i]['timestamp']
            else:
                # Fallback: estimate end time based on mean trial duration from previous trials
                if i > 0:
                    prev_durations = [trials[j]['duration'] for j in range(len(trials))]
                    mean_duration = np.mean(prev_durations)
                    end_time = start_time + mean_duration
                else:
                    # Very first and only trial - use a default duration
                    end_time = start_time + 5.0  # 5 second default

                # Find corresponding end_frame
                time_diffs_to_end = np.abs(eye_df['timestamp'].values - end_time)
                closest_idx = np.argmin(time_diffs_to_end)
                end_frame = eye_df.iloc[closest_idx]['frame']
...
def analyze_feedback_control(trials: list[dict], results_dir: Optional[Path] = None,
    ...
    fig : matplotlib.figure.Figure
        The generated 3-panel analysis figure
    """
    if len(trials) == 0:
    ...
    if len(initial_errors_valid) > 3:
        ...
        for i in range(len(bins)-1):
            ...
            if np.sum(mask) > 0:
                ...
                mean_curvature = np.mean(path_curvatures[mask])
                ...
                binned_curvatures.append(mean_curvature)
                binned_efficiencies.append(mean_efficiency)
                bin_counts.append(np.sum(mask))
            else:
                binned_success_rates.append(np.nan)
                binned_final_errors.append(np.nan)
                binned_curvatures.append(np.nan)
                binned_efficiencies.append(np.nan)
                bin_counts.append(0)
    else:
    ...
    results = {
        ...
        'binned_curvatures': binned_curvatures,
        'binned_efficiencies': binned_efficiencies,
    ...
    # Panel 1: Correlation between initial and final errors
    ax1 = axes[0]
    ax1.scatter(initial_errors_valid, final_errors_valid, alpha=0.6, s=50, edgecolors='black', linewidth=0.5)
    ax1.set_xlabel('Initial Direction Error (°)', fontsize=12)
    ax1.set_ylabel('Final Position Error (stimulus units)', fontsize=12)
    ...
    ax1.set_title(f'Initial vs Final Error\n({trial_selection_label})', fontsize=12, fontweight='bold')
    ax1.grid(True, alpha=0.3)

    # Panel 2: Path curvature by initial error tertile
    ax2 = axes[1]
    if binned_curvatures is not None and bin_labels is not None:
        x_pos = np.arange(len(bin_labels))
        bars = ax2.bar(x_pos, binned_curvatures, alpha=0.7, edgecolor='black', linewidth=1.5)
        ax2.set_xticks(x_pos)
        ax2.set_xticklabels(bin_labels)
        ax2.set_xlabel('Initial Direction Error Tertile', fontsize=12)
        ax2.set_ylabel('Mean Path Curvature (rad)', fontsize=12)
        ax2.set_title(f'Path Curvature vs Initial Error\n({trial_selection_label})', fontsize=12, fontweight='bold')
        ax2.grid(True, alpha=0.3, axis='y')

        # Add sample sizes
        for i, (pos, count) in enumerate(zip(x_pos, bin_counts)):
            ax2.text(pos, 0.02, f'n={count}', ha='center', va='bottom',
    ...
    # Panel 3: Path efficiency by initial error tertile
    ax3 = axes[2]
    if binned_efficiencies is not None and bin_labels is not None:
        x_pos = np.arange(len(bin_labels))
        bars = ax3.bar(x_pos, binned_efficiencies, alpha=0.7, edgecolor='black', linewidth=1.5, color='orange')
        ax3.set_xticks(x_pos)
        ax3.set_xticklabels(bin_labels)
        ax3.set_xlabel('Initial Direction Error Tertile', fontsize=12)
        ax3.set_ylabel('Mean Path Efficiency', fontsize=12)
        ax3.set_title(f'Path Efficiency vs Initial Error\n({trial_selection_label})', fontsize=12, fontweight='bold')
        ax3.set_ylim([0, 1])
        ax3.grid(True, alpha=0.3, axis='y')

        # Add sample sizes
        for i, (pos, count) in enumerate(zip(x_pos, bin_counts)):
            ax3.text(pos, 0.02, f'n={count}', ha='center', va='bottom',
                    transform=ax3.get_xaxis_transform(), fontsize=9)
    else:
    ...
    # Overall title
    title = f'Feedback Control Analysis - {trial_selection_label}'
    if animal_id:
        title += f' - {animal_id}'
    if session_date:
        title += f' ({session_date})'
